...
def calculate_token_size(text, model="gpt-4o-mini"):
    """
            Calculate the token size of a given text for a specified model.
            """
...
def answer_user_question(question, stock_data, financial_data, news_data, max_chunk_size=16000):
    ...
    combined_answers = "\n".join(all_responses)
    token_size = calculate_token_size(combined_answers)
    print(f"Token size of combined answers: {token_size}")
        ...
        messages=[
            ...
            },
            {
                ...
                "content": (
                    ...
                    "3. Assess the sentiment analysis results (positive, negative, or neutral sentiment) and summarize it.\n"
    ...
    token_size = calculate_token_size(chain_of_thought_response.choices[0].message.content)
    print(f"Token size of cot answers: {token_size}")
